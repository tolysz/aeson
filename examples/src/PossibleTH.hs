...
$(deriveJSON defaultOptions { omitNothingFields = True, allowOmittedFields = True } ''Coord)
...
main = do
  ...
  blen $ Coord 3 (-3) MissingData

  blen $ object ["1" .= Missing, "2" .= Null ]
  blen $ object ["a" .= Null , "b" .= Missing, "c" .= Number 1, "d" .= RawNumber "3.5e2" ]
